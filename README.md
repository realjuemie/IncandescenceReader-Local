...
- 管理员可为任一账号创建 5 分钟至 90 天的临时只读分享链接，让未授权游客或会员在有效期内浏览该账号
...
管理员可在 `/admin` 的“用户管理”中点击账号旁的“临时分享”，选择分钟、小时或天数，然后点击“创建并复制”。链接有效期最短 5 分钟、最长 90 天；获得链接的游客或会员只会临时获得该账号的只读访问权，不能进入后台，也不会获得其他隐藏账号权限。到期后链接和已建立的临时访问 Cookie 都会自动失效。
